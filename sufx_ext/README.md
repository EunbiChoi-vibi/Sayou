...
1. 매스(그룹)를 하나 선택하고 **Convert**를 누르면 인터랙티브 격자 툴이 시작됩니다.
   ...
   - B/L: Base/Leg 토글(둘 중 하나만 켤 수 있음), +/-: 높이 5mm씩 조정(최소 5mm,
     기본값 Base 60mm/Leg 100mm). 켜져 있으면 파란 틴트로 바닥 기준 예약 영역을
     미리 보여주고, Enter로 확정하면 바디 그리드가 그만큼 줄어들며 Base(전체
     풋프린트 지지대, 도어 전면 끝선까지 돌출) 또는 Leg(모서리 안쪽 80mm D40
     원기둥 4개 + 10T 전면 가림막)가 함께 생성됩니다.
2. 생성된 `SUFX_BODY#n` 블럭을 선택해 **Merge**(동일 타입+인접 병합) / **Div H, Div V**(N등분)로 다듬습니다.
   Base/Leg 가림막(valance)도 동일 타입끼리 인접하면 Merge로 합칠 수 있습니다.
